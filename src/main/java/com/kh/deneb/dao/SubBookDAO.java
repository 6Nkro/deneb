...
public interface SubBookDAO {
    ...
    @Insert("insert into sub_book (book_seq, parent_bookcase_seq, book_name, book_color, book_icon, book_icon_color, book_type, bookmark_order) ${value}")
    ...
    @Select("select * from sub_book where parent_bookcase_seq in (${parent_bookcase_seq}) order by decode (book_seq ${order})")
